package com.cbd5.resource.other;
import java.util.Iterator;

import javax.ws.rs.GET;
import javax.ws.rs.Path;
import javax.ws.rs.Produces;
import javax.ws.rs.QueryParam;
import javax.ws.rs.core.MediaType;

import com.cbd5.Application;
import com.cbd5.PostgresCommon;
import com.cbd5.TestBean;

import net.sf.json.JSONArray;
import net.sf.json.JSONObject;
/**
 * 蔬菜采收表上报信息汇总
 * @author vicent
 *
 */
@Path("/cszs")
public class Sccsb_UploadSummary {

	PostgresCommon pc = Application.pc;
	@Path("up_summary")
	@GET
	@Produces(value=MediaType.TEXT_PLAIN)
    public String getJson(@QueryParam("time") String time){
		time = time.replaceAll("/", "");
		
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT ");
		sql.append("ROW_NUMBER() OVER() AS 序号,farm AS 基地,filename AS 文件名称,active_time AS 入库时间 ");
		sql.append("FROM ");
		sql.append("(");
		sql.append(" SELECT ");
		sql.append("DISTINCT farm AS farm,filename AS filename,active_time AS active_time");
		sql.append(" FROM ");
		sql.append("originaldata ");
		sql.append("WHERE ");
		sql.append("insert_time ~*");
		sql.append("'"+time+"'");
		sql.append(" ORDER BY 1");
		sql.append(" ) AS t1");
		JSONArray json = pc.resultSetToJson(sql.toString());
//		System.out.println(json.toString());
		
		
		StringBuffer div = new StringBuffer();
		div.append("<table class=\"uk-table\">");
		div.append("<thead>");
		div.append("<tr>");
		
		StringBuffer tbody = new StringBuffer();
		tbody.append("<tbody>");
		for(int i=0; i<json.size(); i++){
			JSONObject j = (JSONObject)json.get(i);
//			System.out.println(j.toString());
			
			Iterator<?> obj=j.keys(); 
			tbody.append("<tr>");
			while (obj.hasNext()) {// 遍历JSONObject
				String key = (String) obj.next().toString();
				if( i == 0 ){
					div.append("<th>"+key+"</th>");
				}
				String data = j.getString(key);
				tbody.append("<td>"+data+"</td>");
			}
			if( i == 0 ){
				div.append("</tr>");
			}
			tbody.append("</tr>");
		}
		tbody.append("</tbody>");
		div.append(tbody.toString());
		div.append("</table>");
		
    	return div.toString();
    }
	
}