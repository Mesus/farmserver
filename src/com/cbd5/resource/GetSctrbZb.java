...
//		String key = "SctrbZb:"+type+"#"+farm+"#"+time;
...
//			Method[] methods = gsz.getClass().getDeclaredMethods();
//			for(int i = 0; i < methods.length; i++){
//				if(methods[i].getName().equals(m_name)){
//					try {
//						methods[i].setAccessible(true);
//						array = (JSONArray)methods[i].invoke(gsz, new Object[]{farm,time});
//					} catch (Exception e) {
...
//		System.out.println(j.toString());
	...
	private JSONArray zb(String farm,String time,String type){
		...
		StringBuffer outout = new StringBuffer();
		StringBuffer outside = new StringBuffer();
		outout.append("SELECT DISTINCT kcb.pmbm as 品名代码,");
		outside.append("(SELECT DISTINCT b1.pmbm as pmbm,");
		...
		if("种子".equals(type)){
			...
			outside.append("b1.pl as pl,b1.pm as pm,b1.spm as spm,");
			b1.append("t1.pl as pl,t1.pm as pm,t1.spm as spm,");
		}
		if("生物制剂".equals(type)){
			outout.append("kcb.pm as 生物制剂品名,kcb.spm as 商品名,");
			outside.append("b1.pm as pm,b1.spm as spm,");
...
//			}
		...
		outside.append("b1.gys as gys,b1.gg as gg,b1.dw as dw,");
...
//		outside.append("b0.cksl as cksl,b0.ckzj AS ckzj,");
		...
		if(one_rday){
			...
			outside.append("(b2.kcsl + b1.rksl - b0.cksl) AS kcsl,(b2.kczj+b1.rkzj-b0.ckzj) AS kczj");
		}
		outout.append(" FROM ");
...
//		sbsql.append(" AND t3.s_type='"+s_type+"'");
		...
		b1.append(" o_timeflag>='"+t_start+"' and o_timeflag<='"+time_arr[1]+"')");
//		sbsql.append(" AND t2.s_rkl+t3.s_ckl > 0");
		...
		b0.append(" o_timeflag>='"+t_start+"' and o_timeflag<='"+time_arr[1]+"') ");
...
//			b2.append(" AND abs(b2.kcsl)+abs(b1.rksl)+abs(b0.cksl)>0 ");
		...
		StringBuffer crksb = new StringBuffer();
		crksb.append("(SELECT DISTINCT b1.pmbm as pmbm,");
		crksb.append("b1.rksl AS rksl,");
		crksb.append("b1.rkzj AS rkzj,");
		crksb.append("b0.cksl AS cksl,");
		crksb.append("b0.ckzj AS ckzj");
		crksb.append(" FROM ");
		crksb.append(b1.toString());
		crksb.append(b0.toString());
		
		crksb.append(" WHERE ");
		crksb.append("b1.pmbm = b0.pmbm ");
		crksb.append(")");
		crksb.append(" AS crkb");
		String crkstr = crksb.toString();
		String ot = "o_timeflag>='"+t_start;
		String nt = "o_timeflag>='"+time_arr[0];
		crkstr = crkstr.replaceAll(ot, nt);
		
		outout.append(outside.toString());
		outout.append(crkstr);
		outout.append(" WHERE ");
		outout.append("kcb.pmbm = crkb.pmbm ");
		outout.append("and abs(crkb.rksl)+abs(crkb.cksl)+abs(kcb.kcsl)>0");
		System.out.println(type+"出入库总表:"+outout.toString());
		array = pc.resultSetToJson(outout.toString());
		return array;
	}
	/**
	 * 创建盘库临时表
